...
class ModelCatalogAridiusNews extends Model {
	...
	public function getaridius_newsShort($limit) {
		...
		if (!$aridius_news_data) {
			...
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "aridius_news n LEFT JOIN " . DB_PREFIX . "aridius_news_description nd ON (n.aridius_news_id = nd.aridius_news_id) LEFT JOIN " . DB_PREFIX . "aridius_news_to_store n2s ON (n.aridius_news_id = n2s.aridius_news_id) WHERE nd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND n2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND n.status = '1'  ORDER BY n.sort_order, n.aridius_news_id DESC LIMIT " . (int)$limit);
	...
	public function getProductRelated($aridius_news_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "aridius_news_product_related nr LEFT JOIN " . DB_PREFIX . "aridius_news n ON (nr.aridius_news_id = n.aridius_news_id) LEFT JOIN " .          DB_PREFIX . "product_to_store p2s ON (nr.product_id = p2s.product_id) WHERE nr.aridius_news_id = '" . (int)$aridius_news_id . "' AND n.status = '1' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");
