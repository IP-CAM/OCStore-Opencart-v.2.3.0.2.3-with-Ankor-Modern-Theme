<?php
class ControllerInformationAridiusNews2 extends Controller {

	public function index() {
		
		$this->language->load('information/aridius_news2');
		...
		if (isset($this->request->get['aridius_news_id2'])) {
			$aridius_news_id2 = $this->request->get['aridius_news_id2'];
		} else {
			$aridius_news_id2 = 0;
		}

		$aridius_news_info = $this->model_catalog_aridiusnews2->getaridius_newsStory($aridius_news_id2);

		if ($aridius_news_info) {
		
			$data['breadcrumbs'][] = array(
				'text'		=> $this->language->get('heading_title'),
				'href'		=> $this->url->link('information/aridius_news2'),
			);

			$data['breadcrumbs'][] = array(
				'text'		=> $aridius_news_info['title'],
				'href'		=> $this->url->link('information/aridius_news2', 'aridius_news_id2=' . $this->request->get['aridius_news_id2']),
			...
			$this->document->addLink($this->url->link('information/aridius_news2', 'aridius_news_id2=' . $this->request->get['aridius_news_id2']), 'canonical');
			...
			$data['addthis'] = $this->config->get('aridius_news2_aridius_newspage_addthis');
			$data['modern_share'] = $this->config->get('modern_share');

			$data['min_height'] = $this->config->get('aridius_news2_thumb_height');
			...
			$data['aridius_news2_status_review'] = $this->config->get('aridius_news2_status_review');

			$data['thumb'] = $this->model_tool_image->resize($aridius_news_info['image'], $this->config->get('aridius_news2_thumb_width'), $this->config->get('aridius_news2_thumb_height'));
     		$data['button_news'] = $this->language->get('button_news');
			$data['button_continue'] = $this->language->get('button_continue');
			$data['aridius_news2'] = $this->url->link('information/aridius_news2');
			$data['continue'] = $this->url->link('common/home');

			$this->model_catalog_aridiusnews2->updateViewed($this->request->get['aridius_news_id2']);
			...
			if (($this->config->get('aridius_news2_review_guest') !=1) || $this->customer->isLogged()) {
			...
			// Captcha
			if ($this->config->get($this->config->get('config_captcha') . '_status') && ($this->config->get('aridius_news2_display_captcha')!=1)) {
			...
			$data['aridius_news_id2'] = (int)$this->request->get['aridius_news_id2'];
			
            // RELATED products
			$data['products'] = array();
			
			$results = $this->model_catalog_aridiusnews2->getProductRelated($this->request->get['aridius_news_id2']);
...
// RELATED aridius_news2
	
		$data['aridius_news2'] = array();

		$results = $this->model_catalog_aridiusnews2->getaridius_newsRelated($this->request->get['aridius_news_id2']);
			foreach ($results as $result) {

		if ($result['image']) {
					$image = $this->model_tool_image->resize($result['image'], $this->config->get('aridius_news2_thumb_width'), $this->config->get('aridius_news2_thumb_height'));
				} else {
					$image = $this->model_tool_image->resize('placeholder.png', $this->config->get('aridius_news2_thumb_width'), $this->config->get('aridius_news2_thumb_height'));
				}
					if ($result['video']) {
					$video = $result['video'];
				} else {
				...
				$data['aridius_news2'][] = array(
					'aridius_news_id2'     => $result['aridius_news_id2'],
					'description' => utf8_substr(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')), 0, $this->config->get($this->config->get('config_theme') . '_product_description_length')) . '..',
				    'image'       => $image,
					'video'       => $video,
					'title'       => $result['title'],
					'viewed'      => $result['viewed'],
					'href'  => $this->url->link('information/aridius_news2', 'aridius_news_id2=' . $result['related_aridius_news_id']),
					'posted'   	  => date($this->language->get('date_format_short'), strtotime($result['date_added']))
				);
			}

// RELATED aridius_news2	end		

			// Template
			$data['aridius_news2_show_date'] = $this->config->get('aridius_news2_show_date');
            ...
            $this->response->setOutput($this->load->view('information/aridius_news2', $data));
   	...
   	$url = '';
		...
		if (isset($this->request->get['limit'])) {
			...
			$limit = (int)$this->config->get($this->config->get('config_theme') . '_product_limit');
				...
				$total = $this->model_catalog_aridiusnews2->getTotalaridius_news();
				...
				$pagination->url = $this->url->link('information/aridius_news2', $url . '&page={page}');
			...
			if ($page == 1) {
			    $this->document->addLink($this->url->link('information/aridius_news2', '', true), 'canonical');
			} elseif ($page == 2) {
			    $this->document->addLink($this->url->link('information/aridius_news2', '', true), 'prev');
			} else {
			    $this->document->addLink($this->url->link('information/aridius_news2', $url . '&page='. ($page - 1), true), 'prev');
			}

			if ($limit && ceil($total / $limit) > $page) {
			    $this->document->addLink($this->url->link('information/aridius_news2', $url . '&page='. ($page + 1), true), 'next');
			}
			...
			foreach($limits as $value) {
				$data['limits'][] = array(
					'text'  => $value,
					'value' => $value,
					'href'  => $this->url->link('information/aridius_news2', $url . '&limit=' . $value)
				);
			}		

			$aridius_news_data = $this->model_catalog_aridiusnews2->getaridius_news($data);
			...
			'href'		=> $this->url->link('information/aridius_news2'),
				...
				$chars = $this->config->get('aridius_news2_headline_chars');
				...
				if ($result['video']) {
					...
					$data['aridius_news_data'][] = array(
						...
						'thumb'				=> $this->model_tool_image->resize($result['image'], $this->config->get('aridius_news2_thumb_width'), $this->config->get('aridius_news2_thumb_height')),
						'video'				=> $video,
						'posted'			=> date($this->language->get('date_format_short'), strtotime($result['date_added']))
					);
				}
	...
	public function review() {
		...
		$this->load->language('information/aridius_review_news2');

	    $this->load->model('catalog/aridiusreviewnews2');

		$data['text_no_reviews'] = $this->language->get('text_no_reviews');
		$data['text_write_reply'] = $this->language->get('text_write_reply');
		
        $data['aridius_news2_display_answer_button'] = $this->config->get('aridius_news2_display_answer_button');
		$limit = $this->config->get('aridius_news2_limite_page');
		...
		$review_total = $this->model_catalog_aridiusreviewnews2->getTotalReviewsByNewsId($this->request->get['aridius_news_id2']);

		$results = $this->model_catalog_aridiusreviewnews2->getReviewsByNewsId($this->request->get['aridius_news_id2'], ($page - 1) * $limit, $limit);

        foreach ($results as $result) {
			
            $parent = $this->model_catalog_aridiusreviewnews2->getReviewsByNewsId($this->request->get['aridius_news_id2'],0,10,$result['review_id']);
		...
		$pagination->page = $page;
		$pagination->limit = $limit;
		$pagination->url = $this->url->link('information/aridius_news2/review', 'aridius_news_id2=' . $this->request->get['aridius_news_id2'] . '&page={page}');
		...
		$this->response->setOutput($this->load->view('information/aridius_review_news2', $data));
	}
	 ...
	 public function reply(){
        ...
        $json['html'] = ($this->load->view('information/aridius_review_news_form2', $data));
	...
	public function write() {
		...
		if (!isset($json['error'])) {
				...
				$this->model_catalog_aridiusreviewnews2->addReviewNews($this->request->get['aridius_news_id2'], $this->request->post);
