...
class ControllerInformationCertificates extends \app\core\Controller {
	...
	public function index(){
		...
		$total = Certificate::totalCount();
		$items = Certificate::getActiveList();
        $data['breadcrumbs'][] = array(
			...
			foreach ($items as $result) {
				...
				$data['items'][] = array(
					...
					'description' => utf8_substr(strip_tags(html_entity_decode($result->description, ENT_QUOTES,
						'UTF-8')), 0, 55),
					'href' => $this->url->link('information/certificates/info', 'id=' . $result->id),
				);
			}
		...
		foreach ($limits as $value) {
			$data['limits'][] = array(
				'text' => $value,
				'value' => $value,
				'href' => $this->url->link('information/certificates', $url . '&limit=' . $value)
			);
		}
		...
		$pagination->url = $this->url->link('information/certificates', $url . '&page={page}');
		...
		if ($page == 1) {
			$this->document->addLink($this->url->link('information/certificates', '', true), 'canonical');
		} elseif ($page == 2) {
			$this->document->addLink($this->url->link('information/certificates', '', true), 'prev');
		} else {
			$this->document->addLink($this->url->link('information/certificates', '&page=' . ($page - 1), true), 'prev');
		}

		if ($limit && ceil($total / $limit) > $page) {
			$this->document->addLink($this->url->link('information/certificates', '&page=' . ($page + 1), true), 'next');
		}
        ...
        $this->response->setOutput($this->load->view('art/certificates/list', $data));
    }

	public function info(){
		$this->language->load('information/certificates');
		...
		$item = Certificate::findOneById($id);
		...
		if ($item) {
			...
			$data['breadcrumbs'][] = array(
				'text' => $item->title,
				'href' => $this->url->link('information/certificates/info', 'id=' . $id)
			);

			$this->document->addLink($this->url->link('information/certificates', 'id=' . $this->request->get['id']),
				'canonical');
			$this->load->model('tool/image');
			...
			$this->response->setOutput($this->load->view('art/certificates/item', $data));
