...
<?php if ($orders) { ?>
	<?php foreach ($orders as $order) { ?>
		...
		<td class="text-left"><a href="<?php echo $order['product_href']; ?>"><?php echo $order['product_name']; ?></a></td>
...
function filter() {
	url = 'index.php?route=catalog/aridiusfastorder&token=<?php echo $token; ?>';
...
$('input[name=\'filter_product_name\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
url: 'index.php?route=catalog/aridiusfastorder/autocomplete&token=<?php echo $token; ?>&filter_product_name=' +  encodeURIComponent(request),
dataType: 'json',
