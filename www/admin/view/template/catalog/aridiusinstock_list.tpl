...
<button type="submit" form="form-notify" data-toggle="tooltip" title="<?php echo $button_module_send_email; ?>" class="btn btn-primary"><i class="fa fa-envelope"></i></button>
...
<div class="form-group">
<label class="control-label" for="input-total"><?php echo $text_email; ?></label>
<input type="text" name="filter_email" value="<?php echo $filter_email; ?>"  id="input-email" class="form-control" />
</div> 
</div>
<div class="col-sm-4">  
<div class="form-group">
<label class="control-label" for="input-name"><?php echo $text_product_name; ?></label>
...
<div class="clearfix">   </div>
<form action="<?php echo $notify; ?>" id="form-notify" method="POST"></form>
...
function filter() {
	url = 'index.php?route=catalog/aridiusinstock&token=<?php echo $token; ?>';
...
$('input[name=\'filter_product_name\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
url: 'index.php?route=catalog/aridiusinstock/autocomplete&token=<?php echo $token; ?>&filter_product_name=' +  encodeURIComponent(request),
dataType: 'json',
