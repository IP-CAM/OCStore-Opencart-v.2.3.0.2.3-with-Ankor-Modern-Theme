 		  ...
 		  <div class="form-group">
                ...
                <div class="col-sm-3">
                  <div class="input-group date">
                    <input type="text" name="date_added" value="<?php echo $date_added; ?>" placeholder="Enter Date" data-date-format="YYYY-MM-DD" id="input-date-available" class="form-control" />
...
<div id="aridius_news2-related" class="well well-sm" style="height: 150px; overflow: auto;">
<?php foreach ($aridius_news_related2 as $aridius_news_related2) { ?>
	<div id="aridius_news2-related<?php echo $aridius_news_related2['product_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $aridius_news_related2['name']; ?>
	<input type="hidden" name="aridius_news_related2[]" value="<?php echo $aridius_news_related2['product_id']; ?>" />
	</div>
	<?php } ?>
</div>
</div>
...
<div id="aridius_news2-aridius_newsrelated" class="well well-sm" style="height: 150px; overflow: auto;">
<?php foreach ($aridius_news_aridius_newsrelated as $aridius_news_aridius_newsrelated) { ?>
	<div id="aridius_news2-aridius_newsrelated<?php echo $aridius_news_aridius_newsrelated['aridius_news_id2']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $aridius_news_aridius_newsrelated['title']; ?>
	<input type="hidden" name="aridius_news_aridius_newsrelated[]" value="<?php echo $aridius_news_aridius_newsrelated['aridius_news_id2']; ?>" />
	</div>
	<?php } ?>
</div>
</div>
...
value: item['product_id']
	...
	'select': function(item) {
		...
		$('#aridius_news2-related' + item['value']).remove();
		$('#aridius_news2-related').append('<div id="aridius_news2-related' + item['value'] + '"><i class="fa fa-minus-circle"></i> ' + item['label'] + '<input type="hidden" name="aridius_news_related2[]" value="' + item['value'] + '" /></div>');	
	}	
});
$('#aridius_news2-related').delegate('.fa-minus-circle', 'click', function() {
	$(this).parent().remove();
});
//--></script> 
<script type="text/javascript"><!--
$('input[name=\'aridius_newsrelated\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
url: 'index.php?route=extension/module/aridius_news2/autocomplete&token=<?php echo $token; ?>&filter_aname=' +  encodeURIComponent(request),
dataType: 'json',			
success: function(json) {
				response($.map(json, function(item) {
					return {
label: item['title'],
value: item['aridius_news_id2']
					}
				}));
			}
		});
	...
	'select': function(item) {
		...
		$('#aridius_news2-aridius_newsrelated' + item['value']).remove();
		$('#aridius_news2-aridius_newsrelated').append('<div id="aridius_news2-aridius_newsrelated' + item['value'] + '"><i class="fa fa-minus-circle"></i> ' + item['label'] + '<input type="hidden" name="aridius_news_aridius_newsrelated[]" value="' + item['value'] + '" /></div>');	
	}	
});
$('#aridius_news2-aridius_newsrelated').delegate('.fa-minus-circle', 'click', function() {
	$(this).parent().remove();
});
//--></script> 
<script type="text/javascript"><!--
